...
export function createMarkdown() {
  ...
  // Drafting notes written as HTML comments stay in the source file for the
  // author and never reach the published page. Only a block that is nothing
  // but a comment is dropped, so mixed HTML keeps rendering untouched.
  const isOnlyComment = (s) => /^<!--[\s\S]*-->$/.test(s.trim());
  for (const rule of ["html_block", "html_inline"]) {
    const next = md.renderer.rules[rule];
    md.renderer.rules[rule] = (tokens, idx, options, env, self) =>
      isOnlyComment(tokens[idx].content) ? "" : next(tokens, idx, options, env, self);
  }
