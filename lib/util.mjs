...
/** Resolve a theme's accent colour: explicit map first, else a stable pick
 *  from the fallback palette (hashed by slug so it never changes). */
export function themeColor(name, colors = {}, palette = ["#4c70b1"]) {
  const key = slugify(name);
  for (const [k, v] of Object.entries(colors)) {
    if (slugify(k) === key) return v;
  }
  let h = 0;
 ...
 *  Everything is anchored to local noon so the local getters used for
 *  display can never slip into the neighbouring day. */
export function toDate(value) {
  if (value instanceof Date) {
    // YAML date-only scalars (`date: 2026-09-10`) reach us as UTC midnight,
    // which reads as the previous day in any negative-offset timezone.
    const dateOnly =
      value.getUTCHours() === 0 &&
...
/** Compact date for tight rows: "5 set 2026". */
export function formatDateShort(value) {
  const d = toDate(value);
  if (!d) return "";
  const month = MONTHS_PT[d.getMonth()].slice(0, 3);
