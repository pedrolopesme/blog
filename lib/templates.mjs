...
export function articleMain(ctx, post, related = [], prev = null, next = null, quote = null) {
  ...
  // Share links (absolute URL for social targets)
  const postAbs = encodeURI(`${(ctx.site.siteUrl || "").replace(/\/$/, "")}${ctx.url(post.url)}`);
  const postTitle = encodeURIComponent(post.title);
  const shareLinks = [
    { label: "LinkedIn", href: `https://www.linkedin.com/sharing/share-offsite/?url=${postAbs}` },
    { label: "X", href: `https://x.com/intent/tweet?url=${postAbs}&text=${postTitle}` },
    { label: "WhatsApp", href: `https://wa.me/?text=${postTitle}%20${postAbs}` },
  ].map((s) => `<a href="${escapeHtml(s.href)}" target="_blank" rel="noopener" class="post-share__link">${escapeHtml(s.label)}</a>`).join("");
