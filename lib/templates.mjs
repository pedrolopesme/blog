...
/** A horizontal strip of themes, echoing Increment's issue navigation. */
function themesStrip(ctx, categories) {
  if (!categories?.length) return "";
  const items = categories
    .map(
      (c) =>
        `<li><a style="--tc:${escapeHtml(c.color)}" href="${ctx.url(`/categorias/${c.slug}/`)}"><span class="themes-strip__count">${c.count}</span>${escapeHtml(c.name)}</a></li>`,
    )
    .join("\n          ");
  return `<nav class="themes-strip wrap" aria-label="Temas">
      <span class="eyebrow-label">Temas</span>
      <ul>
          ${items}
      </ul>
      <a class="u-arrow" href="${ctx.url("/categorias/")}">todos os temas</a>
    </nav>`;
}
...
export function articleMain(ctx, post, related = []) {
  ...
  const reading = post.readingTime
    ? `<span class="eyebrow-sep">·</span><span>${post.readingTime} min de leitura</span>`
    : "";
  ...
  const cover = post.cover
    ? `<figure class="post-cover">${cardCover(ctx, post).replace(/^<div class="article-block__art">|<\/div>$/g, "")}${post.coverCaption ? `<figcaption>${escapeHtml(post.coverCaption)}</figcaption>` : ""}</figure>`
    : "";
  ...
  return `    <article class="post wrap"${accent ? ` style="--tc:${escapeHtml(accent)}"` : ""}>
      <header class="post-header">
        ...
        <p class="post-meta">
          ...
          <span class="eyebrow-sep">·</span>${time}${reading}
        </p>
      </header>
      ${cover}
      <div class="prose">
...
export function homeMain(ctx, { posts, intro, categories }) {
...
${intro}
    ...
    ${themesStrip(ctx, categories)}
    <section class="feed wrap">
      <h2 class="eyebrow-label eyebrow-label--section">Escritos recentes</h2>
