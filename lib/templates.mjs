...
export function articleMain(ctx, post, related = [], prev = null, next = null, quote = null) {
        ...
        <ul class="related-grid">
          ${related.map((p) => {
            ...
            <a href="${url}" class="related-card__link">
              ${art ? `<div class="related-card__img">${art}</div>` : ""}
              <div class="related-card__body">
                ${cat ? `<p class="eyebrow"><span class="eyebrow-theme" style="--tc:${escapeHtml(cat.color)}">${escapeHtml(cat.name)}</span></p>` : ""}
                <h3 class="related-card__title">${escapeHtml(p.title)}</h3>
                ${p.readingTime ? `<p class="related-card__meta">${p.readingTime} min de leitura</p>` : ""}
              </div>
            </a>
          </li>`;}).join("\n          ")}
        </ul>
  ...
  function postNavCard(p, direction) {
    if (!p) return `<div class="post-nav__item post-nav__item--empty"></div>`;
    const art = p.cover ? cardCover(ctx, p).replace(/^<div class="article-block__art">|<\/div>$/g, "") : "";
    const cat = p.categories?.[0];
    return `<a class="post-nav__item post-nav__item--${direction}" href="${ctx.url(p.url)}">
      <span class="post-nav__label">${direction === "prev" ? "← Anterior" : "Próximo →"}</span>
      <span class="post-nav__title">${escapeHtml(p.title)}</span>
      <span class="post-nav__preview">
        ...
        <span class="post-nav__info">
          ...
          ${p.summary ? `<span class="post-nav__excerpt">${escapeHtml(p.summary)}</span>` : ""}
        </span>
      </span>
    </a>`;
  }
  ...
  const postNav = (prev || next)
    ? `<nav class="post-nav wrap" aria-label="Navegação entre posts">
...
${post.html}
      ...
      ${quote ? `<aside class="post-quote">
        <p class="post-quote__label"><span class="post-quote__ornament" aria-hidden="true">❧</span>um pensamento</p>
