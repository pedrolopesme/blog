...
import { escapeHtml, withBase, absoluteUrl, formatDate, formatDateShort, isoDate } from "./util.mjs";
...
export function layout(ctx, page) {
  ...
  const canonical = absoluteUrl(site.siteUrl, site.baseUrl, canonicalPath);
  const image = ogImage || absoluteUrl(site.siteUrl, site.baseUrl, "/assets/logo-512.png");
  const locale = site.language.replace("-", "_");
  const twitterEntry = (site.social || []).find((s) => /(^|\/)(x|twitter)\.com\//i.test(s.href));
  const twitterHandle = twitterEntry ? `@${twitterEntry.href.replace(/\/+$/, "").split("/").pop()}` : null;

  const articleMeta = type === "article"
    ? [
        publishedTime ? `<meta property="article:published_time" content="${escapeHtml(publishedTime)}" />` : "",
        `<meta property="article:author" content="${escapeHtml(site.author.url)}" />`,
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Newsreader:ital,opsz,wght@0,6..72,400;0,6..72,500;0,6..72,600;0,6..72,700;1,6..72,400;1,6..72,500;1,6..72,600&family=Inter:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500&family=Cinzel+Decorative:wght@400&display=swap" rel="stylesheet" />
...
${main}
  ...
  <div class="site-tree" aria-hidden="true">
    <img src="${ctx.asset("tree.webp")}" alt="" loading="lazy" />
  </div>
  <script>(function(){var t=document.querySelector('.site-tree');if(!t)return;var o=new IntersectionObserver(function(e){e[0].isIntersecting&&t.classList.add('is-revealed')},{threshold:.05});o.observe(t);})();</script>
  ${siteFooter(ctx)}
  ...
  <script src="${ctx.asset("code-copy.js")}" defer></script>
</body>
</html>
`;
}
  ...
  return `<header class="site-header">
    <div class="wrap site-header__inner">
      <a class="site-header__brand" href="${ctx.url("/")}">
        <img class="site-header__logo" src="${ctx.asset("logo-lockup.png")}" alt="${escapeHtml(site.title)}" width="476" height="96" />
      </a>
...
export function articleMain(ctx, post, related = [], prev = null, next = null, quote = null) {
  ...
  if (post.date) {
    metaParts.push(
      `<time datetime="${isoDate(post.date)}"><span class="post-meta__date">${formatDate(post.date)}</span><span class="post-meta__date--short">${formatDateShort(post.date)}</span></time>`,
    );
  }
  if (post.readingTime) {
  ...
  if (hasCover) {
    ...
    // Aurora Solar style: pure visual strip, no text overlay.
  ...
  // Sepia info strip — title on the left, meta + share on the right.
  const postHero = hasCover
    ? `<div class="post-hero">
        <div class="post-hero__grid wrap">
          <div class="post-hero__left">
            ${themeLabel ? `<p class="eyebrow">${themeLabel}</p>` : ""}
            <h1 class="post-hero__title">${escapeHtml(post.title)}</h1>
          </div>
          <div class="post-hero__right">
            ${post.summary ? `<p class="post-hero__summary">${escapeHtml(post.summary)}</p>` : ""}
...
export function homeMain(ctx, { posts }) {
  ...
  const heroArt = cardCover(ctx, hero).replace(/^<div class="article-block__art">|<\/div>$/g, "");
  const heroUrl = ctx.url(hero.url);
  const heroCat = hero.categories?.[0];
  const heroDate = hero.date
    ? new Date(hero.date).toLocaleDateString("pt-BR", { month: "long", year: "numeric" })
    : "";

  // Split title: normal part + accent-italic part (last words)
  const titleParts = hero.title.split(/\s+/);
  const accentStart = Math.max(1, titleParts.length - 2);
  const heroTitleNormal = titleParts.slice(0, accentStart).join(" ");
  const heroTitleAccent = titleParts.slice(accentStart).join(" ");

  const heroHtml = `<section class="home-hero wrap">
    <div class="home-hero__bar">
      <div class="home-hero__bar-left">
        <span class="home-hero__bar-tag">§ 01</span>
        <span class="home-hero__bar-sep" aria-hidden="true">•</span>
        <span class="home-hero__bar-label">Destaque</span>
      </div>
      <span class="home-hero__bar-right">Primeiro Plano · Abertura</span>
    </div>
    <div class="home-hero__grid">
      <div class="home-hero__text">
        <div class="home-hero__meta">
          ${heroCat ? `<span class="home-hero__cat">${escapeHtml(heroCat.name)}</span>` : ""}
          ${heroCat && hero.readingTime ? `<span class="home-hero__meta-sep" aria-hidden="true">•</span>` : ""}
          ${hero.readingTime ? `<span class="home-hero__reading">${hero.readingTime} min de leitura</span>` : ""}
        </div>
        <h2 class="home-hero__title">
          <a href="${heroUrl}">${escapeHtml(heroTitleNormal)} <em>${escapeHtml(heroTitleAccent)}</em></a>
        </h2>
        ${hero.summary ? `<p class="home-hero__excerpt">${escapeHtml(hero.summary)}</p>` : ""}
        <div class="home-hero__foot">
          <a class="home-hero__cta" href="${heroUrl}">Ler o ensaio completo<span class="home-hero__arrow" aria-hidden="true">→</span></a>
          ${heroDate ? `<span class="home-hero__date">${escapeHtml(heroDate)}</span>` : ""}
        </div>
      </div>
  ...
  // Layout rotation to break the monotony: cards → list → feature.
  // Single-post groups get the panoramic showcase.
  const rotation = ["cards", "list", "feature"];
  const sections = [];

  // Editorial running order from site.config; unlisted themes keep their
  // natural (newest-first) position, after the ones named there.
  const order = ctx.site.themeOrder || [];
  const rank = (g) => {
    const i = order.indexOf(g.cat.name);
    return i === -1 ? order.length : i;
  };
  const groupList = [...groups.values()];
  groupList.sort((a, b) => rank(a) - rank(b));

  groupList.forEach(({ cat, posts: groupPosts }, idx) => {
    const kicker = (ctx.site.themeKickers || {})[cat.name] || "";
    const num = String(idx + 1).padStart(2, "0");
    const layout = groupPosts.length === 1 ? "panorama" : rotation[idx % rotation.length];
    const catUrl = ctx.url(`/categorias/${cat.slug}/`);
    const count = groupPosts.length;
    const note = layout === "panorama"
      ? "Ensaio de fechamento"
      : `${count} ${count === 1 ? "ensaio" : "ensaios"}`;

    const head = `<div class="home-section__head">
        <div class="home-section__head-left">
          <span class="home-section__num">${num}</span>
          <h2 class="home-section__title">${escapeHtml(cat.name)}</h2>
          ${kicker ? `<span class="home-section__kicker">/ ${escapeHtml(kicker)}</span>` : ""}
        </div>
        <a class="home-section__link" href="${catUrl}">${escapeHtml(note)}<span class="home-section__arrow" aria-hidden="true">→</span></a>
      </div>`;
    ...
    if (layout === "cards") {
      const cards = groupPosts.slice(0, 3).map((p) => {
        ...
        return `<article class="home-card">
          <a class="home-card__art-link" href="${url}">${art}</a>
          <div class="home-card__body">
            <div class="home-card__meta-row">
              <span class="home-card__cat">${escapeHtml(p.categories?.[0]?.name || "")}</span>
    ...
    if (layout === "list") {
      const rows = groupPosts.slice(0, 4).map((p, i) => {
        const url = ctx.url(p.url);
        const art = cardCover(ctx, p).replace(/^<div class="article-block__art">|<\/div>$/g, "");
        return `<article class="home-row">
          ${art ? `<a class="home-row__art" href="${url}">${art}</a>` : ""}
          <div class="home-row__main">
            <span class="home-row__num">${num}.${i + 1}</span>
            <h3 class="home-row__title"><a href="${url}">${escapeHtml(p.title)}</a></h3>
            ${p.summary ? `<p class="home-row__excerpt">${escapeHtml(p.summary)}</p>` : ""}
            <div class="home-row__aside">
              ${p.readingTime ? `<span class="home-row__time">${p.readingTime} min de leitura</span>` : ""}
              <a class="home-row__link" href="${url}">Ler ensaio<span class="home-section__arrow" aria-hidden="true">→</span></a>
            </div>
          </div>
        </article>`;
      }).join("\n        ");
      body = `<div class="home-rows">\n        ${rows}\n      </div>`;
    }

    if (layout === "feature") {
      const [lead, ...minor] = groupPosts;
      const leadUrl = ctx.url(lead.url);
      const leadArt = cardCover(ctx, lead).replace(/^<div class="article-block__art">|<\/div>$/g, "");
      const leadBlock = `<article class="home-feature">
          ${leadArt ? `<a class="home-feature__art" href="${leadUrl}">${leadArt}</a>` : ""}
          <div class="home-feature__meta">
            <span class="home-feature__tag">${escapeHtml(lead.categories?.[0]?.name || "")}</span>
            ${lead.readingTime ? `<span class="home-feature__time">${lead.readingTime} min de leitura</span>` : ""}
          </div>
          <h3 class="home-feature__title"><a href="${leadUrl}">${escapeHtml(lead.title)}</a></h3>
          ${lead.summary ? `<p class="home-feature__excerpt">${escapeHtml(lead.summary)}</p>` : ""}
          <div class="home-feature__foot">
            ${kicker ? `<span class="home-feature__kicker">${escapeHtml(kicker)}</span>` : "<span></span>"}
            <a class="home-feature__link" href="${leadUrl}">Ler na íntegra →</a>
          </div>
        </article>`;
      const minors = minor.slice(0, 3).map((p) => {
        const art = cardCover(ctx, p).replace(/^<div class="article-block__art">|<\/div>$/g, "");
        const url = ctx.url(p.url);
        return `<article class="home-mini">
          ${art ? `<a class="home-mini__art" href="${url}">${art}</a>` : ""}
          <div class="home-mini__text">
            <div class="home-mini__meta">
              <span>${escapeHtml(p.categories?.[0]?.name || "")}</span>
              ${p.readingTime ? `<span>${p.readingTime} min</span>` : ""}
            </div>
            <h4 class="home-mini__title"><a href="${url}">${escapeHtml(p.title)}</a></h4>
          </div>
        </article>`;
      }).join("\n          ");
      body = `<div class="home-split">
        ${leadBlock}
        <div class="home-split__minors">
          ${minors}
        </div>
      </div>`;
    }
    ...
    if (layout === "panorama") {
      ...
      const art = cardCover(ctx, p).replace(/^<div class="article-block__art">|<\/div>$/g, "");
      const url = ctx.url(p.url);
      body = `<article class="home-panorama">
        ${art ? `<a class="home-panorama__art" href="${url}">${art}</a>` : ""}
        <div class="home-panorama__text">
          <div class="home-panorama__meta">
            <span>${escapeHtml(p.categories?.[0]?.name || "")}</span>
            ${p.readingTime ? `<span aria-hidden="true">•</span><span>${p.readingTime} min de leitura</span>` : ""}
          </div>
          <h3 class="home-panorama__title"><a href="${url}">${escapeHtml(p.title)}</a></h3>
          ${p.summary ? `<p class="home-panorama__excerpt">${escapeHtml(p.summary)}</p>` : ""}
          <a class="home-panorama__link" href="${url}">Ler ensaio na íntegra<span class="home-section__arrow" aria-hidden="true">→</span></a>
        </div>
      </article>`;
    }

    sections.push(`<section class="home-section home-section--${layout} wrap">
      ${head}
      ${body}
    </section>`);
  });
